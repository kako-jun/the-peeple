...
技術スタック: **PixiJS 8 + TypeScript + Vite**。縦長ゲーム共通の論理解像度
640×960 (2:3) のキャンバスで描画する。プロジェクト雛形は同じ kako-jun の `amanuma` を
...
縦長ゲーム共通の論理解像度は 640×960 (2:3)。canvas は CSS で拡大せず、`src/main.ts` の `renderer.resize()` と `stage.scale` で viewport に収まる実 canvas サイズへ合わせる。
